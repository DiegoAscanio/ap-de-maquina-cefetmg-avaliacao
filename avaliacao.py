...
class Experimento():
    ...
    def macro_f1_avg(self) -> float:
        ...
        """
        return np.average([self._resultados[i].macro_f1 for i in range(len(self._resultados))])
    @property
    def acuracia(self) -> float:
        """
        Calcula a acuracia dos resultados.
        """
        return np.average([self._resultados[i].acuracia for i in range(len(self._resultados))])
